...
def render_not_found(message: str) -> None:
    ...
    nav_link("pages/1_matches.py", "← ALL MATCHES")
    st.markdown(section_heading("404", "MATCH NOT FOUND", "not-found"), unsafe_allow_html=True)
...
if match is None:
    render_not_found(f"Match {match_id} does not exist in the tournament archive.")
    st.stop()

render_photo_story(
    "DIGITAL MATCH PROGRAMME / 2026",
    "THE MATCH.",
    "REPLAYED.",
    f'{match["Home_Team"]} against {match["Away_Team"]}, from first whistle to final numbers.',
    index=f'MATCH {match_id:02d}',
    page="match-detail",
)
...
st.markdown(
    ...
    '<a href="#timeline">TIMELINE</a><a href="#lineups">LINEUPS</a>'
    '<a href="#players">PLAYERS</a><a href="#facts">FACTS</a><a href="#venue">VENUE</a>'
    '</nav>',
    unsafe_allow_html=True,
)

st.markdown(section_heading("01", "MATCH STATISTICS", "stats"), unsafe_allow_html=True)
st.markdown(render_comparison_stats(match, stats), unsafe_allow_html=True)

st.markdown(section_heading("02", "MATCH TIMELINE", "timeline"), unsafe_allow_html=True)
st.markdown(render_timeline(match, events), unsafe_allow_html=True)

st.markdown(section_heading("03", "TACTICAL LINEUPS", "lineups"), unsafe_allow_html=True)
...
flag_col_config = {"Flag": st.column_config.ImageColumn("Flag", help="National flag", width="small")}
if lineups.empty:
    st.markdown('<div class="match-empty-state">Lineup data unavailable for this match.</div>', unsafe_allow_html=True)
else:
    ...
    def render_team_lineup(team_name: str, team_flag: str) -> None:
        ...
        starters.columns = ["#", "Player", "Position", "Minutes"]
        bench.columns = ["#", "Substitute", "Position", "Minutes"]
        starters.insert(0, "Flag", team_flag)
        bench.insert(0, "Flag", team_flag)
        data_table(starters, width="stretch", label=f"{team_name} / Starting XI",
                   column_config=flag_col_config)
        data_table(bench, width="stretch", label=f"{team_name} / Substitutes",
                   column_config=flag_col_config)
...
st.markdown(section_heading("04", "PLAYER PERFORMANCE", "players"), unsafe_allow_html=True)
...
if motm:
    ...
    if not motm_rows.empty:
        ...
        for column, label in (("Goals", "GOALS"), ("Shots", "SHOTS"), ("Passes", "PASSES")):
            if pd.notna(motm_stats.get(column)):
                value = float(motm_stats[column])
                display_label = label[:-1] if value == 1 else label
                details.append(f'{safe_number(value)} {display_label}')
        motm_team = safe(motm_stats.get("Team"))
    else:
        motm_team = ""
    st.markdown(
        '<div class="motm-programme"><span>PLAYER OF THE MATCH</span><div>'
...
if players.empty:
    st.markdown('<div class="match-empty-state">Player performance data unavailable for this match.</div>', unsafe_allow_html=True)
else:
    ...
    data_table(players, width="stretch", height=520, label="Player match performance index",
               column_config=flag_col_config)

st.markdown(section_heading("05", "MATCH FACTS", "facts"), unsafe_allow_html=True)
st.markdown(render_match_facts(match, stats, players), unsafe_allow_html=True)

st.markdown(section_heading("06", "VENUE", "venue"), unsafe_allow_html=True)
